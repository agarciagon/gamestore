...
<div style="display:grid;grid-template-columns:2fr 1fr;gap:20px;margin-bottom:20px;">
    ...
    <div style="flex:1; display:flex; justify-content:center; align-items:center; padding:16px;"> 
        ...
        <div style="display:flex; align-items:flex-end; gap:12px; height:160px; width:100%; max-width:500px;">
            <?php foreach ($registrosMes as $i => $m):
                $pct = $m['total'] / max($maxMes, 1); // porcentaje real
                $h   = max($pct * 140, 20); // escala altura y mínimo 20px
                $c   = $colors[$i % count($colors)];
            ?>
            <div style="flex:1; display:flex; flex-direction:column; align-items:center; gap:6px;">
                <span style="font-size:12px; font-weight:700; color:var(--text);"><?= $m['total'] ?></span>
                <div style="width:100%; display:flex; align-items:flex-end; height:100%;">
